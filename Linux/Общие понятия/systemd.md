...
2. **Timer** - в этой секции определяется расписание, когда таймер должен запускать связанный с ним юнит (с тем же названием). Расписание может быть абсолютным (определенное время) или относительным (например, "каждые 10 минут"). Вот пример:
